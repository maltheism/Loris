    ...
    <form method="post">
        ...
        {foreach from=$comment item=curr_comment}
            ...
            {section name=predefined loop=$curr_comment.predefined}
                {if $has_permission}
                    <input type="checkbox" name="savecomments[predefined][]" value="{$curr_comment.predefined[predefined].id}"
                           {if $curr_comment.predefined[predefined].checked}checked="checked"{/if}>{$curr_comment.predefined[predefined].predefined_text}
                    <br/>
